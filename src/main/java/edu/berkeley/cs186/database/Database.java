    ...
    private class TransactionContextImpl extends TransactionContext {
        ...
        public void close() {
            try {
                ...
                List<Lock> existLocks = lockManager.getLocks(transaction);
                for (int i = existLocks.size() - 1; i >= 0; i--) {
                    LockContext.fromResourceName(lockManager, existLocks.get(i).name).release(transaction);
                }
                return;
            } catch (Exception e) {
